...
func TestContextToolInjectsStyleStats(t *testing.T) {
	...
	body := "# Chương N\nKhông phải anh trì hoãn, mà là sợ hãi. Hệt như một con dốc dài. Anh dường như im lặng không nói gì.\nĐêm tối buông xuống.\nAnh ấy đi rồi."
	for ch := 1; ch <= 6; ch++ {
...
// Hồi quy sự cố biên cung: ngữ cảnh Người viết từng lấy vị trí từ Progress.CurrentVolume/CurrentArc, vốn
// được commit_chapter ghi SAU mỗi chương nên luôn mô tả cung của chương TRƯỚC. Hậu quả tại mọi biên cung:
// chương đầu cung nhận arc_goal của cung vừa kết thúc, kèm arc_chapter_index vô nghĩa ("chương 11/10").
func TestChapterContextPositionFollowsChapterNotProgress(t *testing.T) {
	s := store.NewStore(t.TempDir())
	if err := s.Init(); err != nil {
		t.Fatalf("Init: %v", err)
	}
	if err := s.Progress.Init("test", 15); err != nil {
		t.Fatalf("InitProgress: %v", err)
	}
	if err := s.Outline.SaveLayeredOutline([]domain.VolumeOutline{{
		Index: 1, Title: "Tập một", Theme: "chủ đề tập",
		Arcs: []domain.ArcOutline{
			{Index: 1, Title: "Cung một", Goal: "mục tiêu cung MỘT", Chapters: outlineEntries(10)},
			{Index: 2, Title: "Cung hai", Goal: "mục tiêu cung HAI", Chapters: outlineEntries(5)},
		},
	}}); err != nil {
		t.Fatalf("SaveLayeredOutline: %v", err)
	}
	...
	// Đúng trạng thái sau khi commit chương 10: Progress trỏ V1/A1, còn chương sắp viết là 11 (thuộc A2).
	...
	tool := NewContextTool(s, References{}, "default", rules.LoadOptions{})
	envelope := chapterContextEnvelope{Episodic: map[string]any{}}
	tool.buildChapterEpisodicMemory(&envelope, contextBuildState{
		chapter:  11,
		profile:  domain.ContextProfile{Layered: true},
		progress: progress,
	}, func(string, error) {})

	pos, ok := envelope.Episodic["position"].(map[string]any)
	if !ok {
		t.Fatalf("thiếu position trong ngữ cảnh: %+v", envelope.Episodic)
	}
	if pos["arc"] != 2 {
		t.Errorf("arc = %v, mong đợi 2: chương 11 là chương đầu Cung hai", pos["arc"])
	}
	if pos["arc_goal"] != "mục tiêu cung HAI" {
		t.Errorf("arc_goal = %q, mong đợi mục tiêu của cung đang viết chứ không phải cung trước", pos["arc_goal"])
	}
	if pos["arc_chapter_index"] != 1 || pos["arc_total_chapters"] != 5 {
		t.Errorf("chương 11 là chương %v/%v của cung, mong đợi 1/5",
			pos["arc_chapter_index"], pos["arc_total_chapters"])
	}
}

func outlineEntries(n int) []domain.OutlineEntry {
	var out []domain.OutlineEntry
	for i := range n {
		out = append(out, domain.OutlineEntry{
			Title: fmt.Sprintf("ch%d", i+1), CoreEvent: "sự kiện", Hook: "móc",
		})
	}
	return out
}
