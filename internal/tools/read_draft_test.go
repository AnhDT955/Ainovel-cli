...
func TestDraftChapterWrite(t *testing.T) {
	...
	// draft_chapter dùng chung guard đề cương với plan_chapter: chương phải có trong outline,
	// và phase phải đạt outline thì StartChapter mới nâng lên writing (khớp luồng save_foundation thật).
...
func TestDraftChapterAppend(t *testing.T) {
	...
	if err := store.Outline.SaveOutline([]domain.OutlineEntry{
		{Chapter: 2, Title: "承接", CoreEvent: "冲突升级", Hook: "危机逼近"},
	}); err != nil {
		t.Fatalf("SaveOutline: %v", err)
	}
...
func TestPlanChapterMarksInProgress(t *testing.T) {
	...
	// plan_chapter yêu cầu chương phải có trong đề cương; giai đoạn writing luôn có đề cương
	// (FoundationMissing chặn phase writing khi thiếu outline).
	if err := store.Outline.SaveOutline([]domain.OutlineEntry{
		{Chapter: 1, Title: "起头", CoreEvent: "建立处境", Hook: "发现线索"},
	}); err != nil {
		t.Fatalf("SaveOutline: %v", err)
	}
	// StartChapter chỉ nâng phase outline→writing, nên phải mô phỏng đúng save_foundation: lưu đề cương xong đặt PhaseOutline.
	if err := store.Progress.UpdatePhase(domain.PhaseOutline); err != nil {
		t.Fatalf("UpdatePhase: %v", err)
	}
	...
	args, _ := json.Marshal(map[string]any{
		...
		"goal":             "建立处境",
		"conflict":         "债务逼近",
		"opening_pressure": "债主堵门",
		"stakes":           "失去自由并连累家人",
		"pressure_chain":   []string{"债主限时签契 → 主角拖延 → 房门被锁", "主角找契约漏洞 → 对方叫来证人 → 必须暴露识字能力"},
		"turning_point":    "契约印章是伪造的",
		"character_choice": "暴露识字能力以保住自由",
		"consequence":      "债主开始怀疑并派人跟踪",
		"hook":             "发现线索",
	})
