...
func TestStartChapter(t *testing.T) {
	...
	store := NewStore(dir)
	_ = store.Progress.Init("test", 10)
	_ = store.Progress.UpdatePhase(domain.PhaseOutline)
...
// StartChapter không được nâng phase từ trước-outline lên writing: nâng vô điều kiện là cách sự cố
// 2026-07-16 khoá máy trạng thái vào writing khi đề cương chưa từng được lưu (phase chỉ tiến không lùi).
func TestStartChapterDoesNotPromotePhaseBeforeOutline(t *testing.T) {
	dir := t.TempDir()
	store := NewStore(dir)
	_ = store.Progress.Init("test", 10)
	_ = store.Progress.UpdatePhase(domain.PhasePremise)
	...
	p, _ := store.Progress.Load()
	if p.Phase != domain.PhasePremise {
		t.Fatalf("expected phase to stay premise, got %s", p.Phase)
	}
	if p.InProgressChapter != 1 {
		t.Fatalf("expected in-progress chapter 1, got %d", p.InProgressChapter)
	}
