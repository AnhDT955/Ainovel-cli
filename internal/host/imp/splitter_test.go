...
func TestSplitText_Vietnamese(t *testing.T) {
	src := `Chương 1: Khởi đầu mới
Gió thổi ngoài hiên, một ngày mới bắt đầu.

Chương 2: Bước đi đầu tiên
Đường dài vạn dặm khởi đầu từ bước chân.

Mở đầu
Đây là phần mở đầu của câu chuyện.

Ngoại truyện: Hồi ức xưa
Ký ức ngọt ngào từ thuở thiếu thời.`
	got := splitText(src, defaultChapterRegex)
	if len(got) != 4 {
		t.Fatalf("want 4 chapters, got %d", len(got))
	}
	if got[0].Title != "Khởi đầu mới" {
		t.Errorf("ch1 title: got %q want %q", got[0].Title, "Khởi đầu mới")
	}
	if got[1].Title != "Bước đi đầu tiên" {
		t.Errorf("ch2 title: got %q want %q", got[1].Title, "Bước đi đầu tiên")
	}
	if got[2].Title != "Mở đầu" {
		t.Errorf("ch3 title: got %q want %q", got[2].Title, "Mở đầu")
	}
	if got[3].Title != "Hồi ức xưa" {
		t.Errorf("ch4 title: got %q want %q", got[3].Title, "Hồi ức xưa")
	}
}
